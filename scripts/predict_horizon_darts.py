...
def main(args):
    ...
    for sub in tqdm(SUBS, desc="subjects"):
        ...
        valid_runs = [
            run
            for run in all_runs
            if data_file[run][:].shape[0] > in_length
        ...
        for run in valid_runs:
            ...
            chunk_list = [
                series_np[k : k + in_length]
                for k in range(series_np.shape[0] - 6 - in_length + 1)
            ]
            ...
            for _ in range(6):
                ...
                for chunk in chunk_list:
                    ...
                    new_chunk_list.append(
                        np.concatenate((chunk.values()[1:], pred.values()))
                    )
                pred_per_lag.append(np.concatenate(pred_list))
                chunk_list = [TimeSeries.from_values(c) for c in new_chunk_list]

            r2 = []
            for lag, pred in enumerate(pred_per_lag):
                r2.append(
                    r2_score(
                        series_np[in_length + lag : len(series_np) - 6 + lag + 1],
                        pred,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Load a model and compute its score on several tasks data."
    )
    parser.add_argument(
