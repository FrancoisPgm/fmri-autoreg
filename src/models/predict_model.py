...
def predict_model(model, params, data_file, task_filter):
    ...
    del data_list
    batch_size = len(X) if "batch_size" not in params else params["batch_size"]
    Z = np.concatenate(
        [model.predict(x) for x in np.array_split(X, ceil(X.shape[0] / batch_size))]
    )
...
def predict_horizon(
    ...
    if not len(X):
        warnings.warn(
            f"No data found in {data_file} for task {task_filter}", RuntimeWarning
        )
        return (None,) * 3
    Z = []
    Y = X[:, :, seq_length:]
...
def main():
    """Use a trained model to predict on data and save R2 score, prediction and labels."""
    model_path = (
        args.model
    ...
    r2, Z, Y = predict_model(model, params, args.data_file, args.task_filter)
    if args.verbosity:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tag", type=str, default="", help="Tag to append to output file names."
    )
