...
def main(args):
    """Mask data, project on atlas and save preprocessed data in an hdf5 file."""
    ...
    with h5py.File(args.out_path, "a") as f:
        for in_path in tqdm(file_list, desc="Applying atlas masks"):
            ...
            if filename not in f.keys():
                brain_mask_path = in_path.replace(
                    "preproc_bold.nii.gz", "brain_mask.nii.gz"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-a", "--atlas", type=str, help="Path to the atlas.", required=True
    )
    parser.add_argument(
        "-o", "--out_path", type=str, help="Output path.", required=True
    )
